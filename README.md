...
`docs/deploy.md` 참조. 두 갈래다 — 주변에 읽혀보는 단계면 Cloudflare Tunnel(0원,
내 컴퓨터가 켜져 있을 때만), 계속 켜둬야 하면 Fly.io(월 5달러).
DB 시드는 앱이 부팅 때 스스로 하므로 컨테이너에 스크립트가 없어도 된다.

### 읽은 기록

`/admin/enter?token=…` 로 한 번 들어가면 그 뒤로는 `/admin`. `ADMIN_TOKEN`
환경변수를 정해두지 않으면 아무도 못 들어온다.

시작·완독·완독률, 이탈 페이지, 문항별 중앙 망설임 시간과 선택 비율, 유형 분포를 본다.
**중앙 망설임이 3초 아래인 문항은 표시된다** — 그 문항은 축을 못 재고 있을 가능성이
크다 (spec §7). 이 숫자들이 다음 작품의 설계를 정한다.
...
홈 화면에 설치되는 앱으로 동작한다. `app/manifest.ts`가 신원을,
`public/sw.js`가 캐시를 맡는다.

캐시 전략은 자원 성격으로 나눈다. 삽화와 `/_next/static`은 URL에 버전이나 해시가
박혀 있으므로 cache-first, 화면은 network-first다 — 서버가 진실이고 끊겼을 때만
마지막으로 본 것을 준다. `mode === "navigate"`만 보면 안 된다: App Router는 첫
진입만 문서 요청이고 이후 Link 이동은 `?_rsc=` 요청이라 그것까지 담아야 오프라인에서
장을 넘길 수 있다.

읽기를 시작하면 그 작품의 나머지 페이지를 조용히 받아둔다. 삽화(작품당 1.8MB)는
표지의 「오프라인으로 저장」으로 명시적으로 받는다 — 셀룰러에서 말없이 2MB를
쓰지 않는다.

오프라인에서 답한 것은 `lib/pending-answers.ts`가 기기에 붙잡아 두고 다시 연결될 때
순서대로 보낸다. Background Sync는 iOS에 없으므로 기대지 않고 앱이 뜰 때와
`online` 이벤트에서 직접 흘려보낸다. 지하철에서 연결이 끊겼다고 이야기가 멈추면 안 된다.
