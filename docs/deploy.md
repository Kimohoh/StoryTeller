...
터널 주소는 껐다 켤 때마다 바뀐다. 그래서 앱은 이렇게 만들어져 있다.

- **앱 안의 이동은 전부 상대 경로다.** 호스트가 무엇이든 그대로 산다.
- **기기에 저장되는 결과는 주소가 아니라 `sessionId`다.** 새 주소로 열어도
  「이 기기에 남은 기록」이 그대로 보인다.
- **바깥으로 나가는 링크는 `/out/<슬러그>`를 거친다.** 목적지를 바꿔도 이미
  공유된 결과 링크가 안 죽고, 클릭도 거기서 센다.
- **공유 링크만 절대 주소가 필요하다.** `APP_ORIGIN`을 정해두면 지금 보고 있는
  호스트가 아니라 그 주소로 공유된다.

```bash
APP_ORIGIN=https://내도메인 ADMIN_TOKEN=... npm start
```

도메인이 생기기 전에는 비워둔다 — 비어 있으면 지금 보고 있는 주소를 쓴다.
Fly로 옮길 때는 `fly secrets set APP_ORIGIN=https://<앱이름>.fly.dev` 한 줄이면 된다.
**터널로 테스트하는 동안 공유된 링크는 그 터널이 살아 있는 동안만 유효하다.**
사람들에게 결과를 저장하라고 안내할 거면 도메인부터 잡는 편이 낫다.
...
재보고 말한다. 서버가 떠 있는 상태에서:

```
npm run bench
```

한 사람이 한 편을 읽는 동안 서버에 닿는 것을 그대로 흉내낸다 — 서재, 표지,
세션 만들기, 장마다 본문과 삽화, 결과. 생각하는 시간만 뺐다.

2026년 9월, 4코어 2.1GHz에서 잰 값(맥북 에어는 이보다 낫다):

| | |
|---|---|
| 한 사람이 한 편 읽는 동안 | **18요청 · 1.48MB** |
| 서버가 버티는 처리량 | **약 240 req/s** (동시 80명까지 실패 0) |

**여기서 '동시 N명'은 쉬지 않고 읽는 N명이다.** 실제 독자는 7장을 8분쯤
걸려 읽으므로 1인당 초당 0.04요청밖에 안 쓴다. 240 req/s면 산술적으로는
동시 수천 명이고, 하루 1만 명은 평균 1.4Mbps짜리 일이다.

### 그래서 진짜 병목은 CPU가 아니라 대역폭이다

1.48MB 중 1.4MB가 삽화다. 집 인터넷 업로드가 100Mbps라면 초당 8명분,
500Mbps면 초당 40명분을 새로 밀어낼 수 있다.

**삽화가 Cloudflare에 캐시되면 이게 거의 사라진다.** 원본이 한 번만 내보내고
그다음부터는 엣지가 답한다. 확인:

```
curl -sI https://godok.page/api/illustrations/the-necklace/p1_invitation | grep -i cf-cache-status
```

- `HIT` 또는 `MISS`가 보이면 캐시 대상이다. 두 번째 요청에서 `HIT`이면 된 것이다
- 아무것도 안 보이면 Cloudflare가 이 경로를 캐시하지 않고 있다. 확장자가 없는
  주소라 기본 규칙에 안 걸린다. 대시보드 → Caching → Cache Rules에서
  `URI Path starts with /api/illustrations/` → Eligible for cache 규칙을 하나
  만들면 된다. 응답에 이미 `immutable`이 붙어 있으므로 안전하다

이 규칙 하나로 원본이 내보내는 양이 1.48MB에서 0.1MB 아래로 떨어진다.

### 실제로 무너지는 건 부하가 아니다

- 노트북이 잠들거나 덮개가 닫힌다
- 집 인터넷이 끊기거나 공유기가 재부팅된다
- 자동 업데이트로 재부팅된다 (launchd가 다시 띄우긴 한다)

부하보다 이 셋이 먼저 온다. 감당 못 할 트래픽이 걱정돼서 홍보를 미룰
이유는 없다.
